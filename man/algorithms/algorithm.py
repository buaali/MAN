...
import man.algorithms.mix_selfsupervision.clustering as clustering
import man.architectures as architectures
import man.utils as utils
from man.algorithms.mix_selfsupervision.fc_cluster import fc_cluster
from man.algorithms.mix_selfsupervision.fc_cluster import FC3
...
class Algorithm:
    def __init__(self, opt,_run=None, _log=None, k = 10, w = 640):
        ...
        self.gcn.set_w(w)
        self.my_fc_cluster = fc_cluster(self.k)
        self.FC3 = FC3(3,512, 256, self.k)
    ...
    def solve(self, data_loader_train , data_loader_test=None):
        ...
        self.deepcluster = clustering.__dict__["Kmeans"](self.k)
    ...
    def evaluate(self, dloader, idx=None):
        ...
        print("k: ",self.k)
        print("gcn : ", sum(x.numel() for x in self.gcn.parameters()))
        print("fc : ", sum(x.numel() for x in self.FC3.parameters()))
        print(sum(x.numel() for x in self.gcn.parameters()) + sum(x.numel() for x in self.FC3.parameters()))
        ...
        for key, network in self.networks.items():
            ...
            #print(network, sum(x.numel() for x in network.parameters()))
