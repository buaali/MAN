...
class CosineClassifierWithWeightGeneration(nn.Module):
    def __init__(
        ...
        weight_base = torch.FloatTensor(self.num_classes, self.num_features).normal_(
        ...
        self.scale_cls = nn.Parameter(
            torch.FloatTensor(1).fill_(scale_cls), requires_grad=self.learn_scale
        )

        self.bias_novel = bias_novel

    def get_classification_weights(self, base_ids, features_train=None, labels_train=None):
        ...
        #pdb.set_trace()
        weight_novel = cutils.average_train_features(features_train, labels_train)
        #pdb.set_trace()
        weight_novel = weight_novel.view(meta_batch_size, num_novel, self.num_features)
        #pdb.set_trace()
        # ***********************************************************************
    ...
    def apply_classification_weights(self, features, classification_weights):
        ...
        #pdb.set_trace()
        if features.dim() == 5:
            features = cutils.preprocess_5D_features(features, self.global_pooling)
        #pdb.set_trace()
        assert features.dim() == 3
        assert features.size(2) == self.num_features

        classification_scores = tools.batch_cosine_fully_connected_layer(
            features, classification_weights.transpose(1, 2), scale=self.scale_cls
        )

        if self.bias_novel is not None:
            classification_scores_base = classification_scores[:, :, : self.num_base]
            classification_scores_novel = (
                self.bias_novel * classification_scores[:, :, self.num_base :]
            )
            classification_scores = torch.cat(
                [classification_scores_base, classification_scores_novel], dim=2
            )
    ...
    def parse_base_ids(self, base_ids, meta_batch_size=1):
        if base_ids is None:
            ...
            base_ids = (
                torch.LongTensor(list(range(num_base))).view(1, -1).repeat(meta_batch_size, 1)
            )
            base_ids = base_ids.to(device)

        return base_ids

    def forward(self, features_test, base_ids=None, features_train=None, labels_train=None, labels=None, batch_size=None, is_train = False):
        ...
        if is_train:
            if self.flag is not None and batch_size is not None:
                classsification_scores = classsification_scores.view(batch_size, -1)
            loss = F.cross_entropy(classsification_scores, labels)
            loss = 2*loss * (torch.sigmoid(self.lamda)+1)
            return classsification_scores, loss
        else:
